...
impl FsReader {
    ...
    /// with the given `node_size` and `base_offset`.  This is the single
    /// sanctioned construction site — call sites go through
    /// [`crate::btrfs::open`], which already knows `node_size` from the
    /// superblock.
    pub fn new(fp: File, node_size: usize, base_offset: u64) -> Self {
        Self { fp, node_size, base_offset }
    }
